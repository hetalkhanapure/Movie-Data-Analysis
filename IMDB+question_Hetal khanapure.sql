...
-- count the total number of rows in movie table
-- total rows : movie : 7997
SELECT COUNT(*) AS rows_movies FROM movie ;

-- count the total number of rows in genre table
-- total rows : genre : 14662
SELECT COUNT(*) AS rows_genre FROM genre ;

-- count the total number of rows in director_mapping table
-- total rows : director_mapping : 3867
SELECT COUNT(*) AS rows_dir FROM director_mapping ;

-- count the total number of rows in names table
-- total rows : names : 25735
SELECT COUNT(*) AS rows_names FROM names ;

-- count the total number of rows in ratings table
-- total rows : ratings : 7997
SELECT COUNT(*) AS rows_ratings FROM ratings ;

-- count the total number of rows in role_mapping table
-- total rows : role_mapping : 15615
SELECT COUNT(*) AS rows_role FROM role_mapping ;



-- Q2. Which columns in the movie table have null values?
-- Ans: country,worlwide_gross_income,languages,production_company have null values in movie table.
-- Type your code below:
...
SELECT COUNT(m.id) AS cnt_genre,
		m.id 
FROM movie AS m
...
SELECT production_company,
	   movie_count,
	   prod_company_rank
FROM prd_company_summary 
HAVING prod_company_rank =1;
...
SELECT COUNT(*) AS movie_count FROM median_rating_detail;
...
WHERE m.languages REGEXP 'German')AS votes_German,
(SELECT SUM(r.total_votes) 
FROM movie AS m
INNER JOIN ratings AS r ON m.id=r.movie_id
WHERE m.languages REGEXP 'Italian')AS votes_Italian;
...
with genre_rank as(
SELECT g.genre as top_genre,count(g.movie_id) 
from genre as g 
...
limit 3),
director_rank as (
select dm.movie_id,n.name,g.genre ,r.avg_rating
from director_mapping as dm
inner join names as n on n.id=dm.name_id
inner join genre as g on g.movie_id=dm.movie_id
inner join ratings as r on r.movie_id=dm.movie_id
where r.avg_rating >8
)
select name as director_name,count(movie_id) as movie_count
from director_rank as dr
inner join genre_rank gr on gr.top_genre=dr.genre
group by name
...
INNER JOIN names AS n ON n.id=rm.name_id
WHERE country REGEXP 'India' AND 
	  category='actor'
GROUP BY n.name)
SELECT * ,
...
SELECT m.id,
       ...
       r.avg_rating,
		...
		END AS Category
FROM movie AS m 
INNER JOIN genre ON genre.movie_id=m.id
INNER JOIN ratings AS r ON r.movie_id=m.id
...
SELECT genre,
        ...
        CAST(REPLACE (worlwide_gross_income,'$','') AS UNSIGNED ) AS gross_income
FROM movie AS m 
INNER JOIN genre AS g ON g.movie_id=m.id
),
FinalResult as (
SELECT genre,
		...
		DENSE_RANK() OVER(Partition by movie_year ORDER BY gross_income DESC) AS movie_rank
FROM movie_detail AS md
INNER JOIN top_genre AS tg ON tg.top_genre=md.genre)
SELECT  * 
FROM FinalResult
WHERE movie_rank <=5
;
...
FROM movie AS m
INNER JOIN ratings AS r ON r.movie_id=m.id
WHERE POSITION(',' IN languages)>0 AND 
...
SELECT dm.name_id AS director_id,
       ...
       avg_rating,
       duration,
	   LAG(duration,1) OVER(PARTITION BY name ORDER BY duration) AS prev_movie_duration
FROM movie AS m
...
SELECT director_id,
		director_name,
        COUNT(id) AS number_of_movies,
        ROUND(AVG(duration - prev_movie_duration)) AS avg_inter_movie_days,
